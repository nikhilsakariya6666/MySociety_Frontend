...
import { getAllEvent, addEditEvent, deleteEventById } from "service/event.service";
...
export default function Event() {
  ...
  const getAll = async () => {
    try {
      const res = await getAllEvent().then((response) => {
        // console.log("response ===>", response);
        if (response && response.status === "success" && response.events_operators) {
          // const arr = response.society_operators.map((item) => [item.societyName]);
          setEventData(response.events_operators);
        } else {
          setEventData([]);
        }
      });
  ...
  const onResetEventObj = async (obj) => {
    setEventObj({
      eventName: (obj && obj.eventName) || "",
  ...
  const onSubmitClick = async () => {
    const response = await addEditEvent(eventObj, eventId);
    console.log("Response", response);
    if (response.status === "success") {
      alert(response.message);
      onResetEventObj({});
                                  ...
                                  <MDBox mb={2}>
                                    ...
                                    >
                                      {/* <MDInput
                                        ...
                                        variant="standard"
                                        fullWidth
                                        defaultValue={eventObj.startDate}
                                        onChange={(e) =>
                                          setEventObj({
                                            ...eventObj,
                                  ...
                                  <MDBox mb={2}>
                                    ...
                                    >
                                      <MDInput
                                        ...
                                        variant="standard"
                                        fullWidth
                                        defaultValue={eventObj.societyId}
                                        onChange={(e) =>
                                          setEventObj({
                                            ...eventObj,
